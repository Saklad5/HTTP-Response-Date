...
final class HTTPResponseDateTests: XCTestCase {
	/// A series of dates along with each supported `date` header representation of them.
	///
    ...
    .map {
      (key: Date(timeIntervalSinceReferenceDate: $0.key), value: $0.value)
    }

	func testRFC1123TimestampParsing() {
    Self.testResponses
      .forEach { XCTAssertEqual($0.key, HTTPURLResponse.date(forDateValue: $0.value.rfc1123)) }
	}
	...
	func testRFC850TimestampParsing() throws {
		try Self.testResponses.forEach {
			let parsedDate = try XCTUnwrap(HTTPURLResponse.date(forDateValue: $0.value.rfc850))

			guard $0.key != parsedDate else { return }
    ...
    Self.testResponses
      .forEach { XCTAssertEqual($0.key, HTTPURLResponse.date(forDateValue: $0.value.asctime)) }
	}
